import pygame
from settingsGame.settings_game import GameSettings, BackTemple, Tiles, world_data3, Player_Sprites, Enemy_Sprites, Items, Sounds
import os
import math
...
from settingsGame.final_boss import FinalBoss
from settingsGame.level_reset_manager import LevelResetManager
from settingsGame.treasure import Treasure
from levels.final_level import FinalLevel
...
class Level3:
    ...
    class Player(pygame.sprite.Sprite):
        ...
        def check_enemy_collision(self):
            ...
            for boss in self.game.boss_group:
                if self.rect.colliderect(boss.rect):
                    # Se tiver escudo ativo, causa dano ao boss
                    if self.shield_active:
                        self.game.audio_manager.play_effect("attack")
                        if boss.take_damage():
                            boss.kill()
                    ...
                    # Se pulou em cima do boss
                    elif self.vel_y > 0 and self.rect.bottom < boss.rect.centery:
                        if not self.is_knockback:
                            self.attack_sound.play()
                            self.vel_y = -15
                            if boss.take_damage():
                        ...
                        if not self.is_knockback and not self.is_invulnerable and not self.shield_active:
                            self.take_damage()
                            return False
        ...
        def update(self, keyboard):
            ...
            if self.is_invulnerable:
                self.invulnerable_timer += 1 #1/60
                if self.invulnerable_timer > 60:  #>= self.invulnerable_duration:
                    self.is_invulnerable = False
                    self.invulnerable_timer = 0
        ...
        #adicionei isso para a colisão com o boss
    ...
    def __init__(self, game):
        self.game = game
        self.traps_group = pygame.sprite.Group() #adiciona o grupo de traps
        self.reset_manager = LevelResetManager(self.game)
        self.final_screen = None
        self.show_final_screen = False
        self.screen_delay = 1000  # 2 segundos de delay
        self.screen_timer = 0
        self.screen = game.window.get_screen()
        ...
        self.audio_manager.set_level(3)
        self.treasure_sound = pygame.mixer.Sound(Sounds["treasure"])
        self.treasure_sound.set_volume(0.5) 

        # Adicione estas duas linhas aqui
        self.show_game_over = False
        self.game_over_screen = None
        
        # Calcula a largura do mundo baseado na matriz world_data3
        self.world_width = len(world_data3[0]) * self.tile_size
        self.world_height = len(world_data3) * self.tile_size
        
        # Agora carrega as camadas do fundo
        self.bg_layers = []
        for i in range(1, 5):  # 4 camadas
            layer = pygame.image.load(BackTemple[f"layer{i}"]).convert_alpha()
        ...
        # Carregando tiles
        self.dirt_img = pygame.image.load(Tiles["dirt3"]).convert_alpha()
        self.grass_img = pygame.image.load(Tiles["grass3"]).convert_alpha()
        ...
        self.theme_sound = pygame.mixer.Sound(Sounds["theme"])
    ...
    def draw(self, show_grid=False):
        ...
        for group in [self.coins_group, self.potions_group, self.keys_group]:
         ...
         # Desenha o boss e seus lasers
        for boss in self.boss_group:
            screen_x = boss.rect.x - self.camera_x
            screen_y = boss.rect.y - self.camera_y
            self.screen.blit(boss.image, (screen_x, screen_y))
            
            # Desenha os lasers do boss
            for laser in boss.lasers:
                laser_x = laser.rect.x - self.camera_x
                laser_y = laser.rect.y - self.camera_y
                self.screen.blit(laser.image, (laser_x, laser_y))
    ...
    def create_boss(self):
      for y, row in enumerate(world_data3):
          for x, tile in enumerate(row):
              if tile == 'B':  # 'B' representa o boss na matriz
                  # Cria apenas um boss com as coordenadas corretas
                  self.boss = FinalBoss(x * self.tile_size, (y * self.tile_size) + 49, self)
                  self.boss_group.add(self.boss)
              if tile == 'T':  # 'T' representa onde o tesouro vai aparecer
                  # Guarda a posição do tesouro para usar depois
                  self.treasure_pos = (x * self.tile_size, y * self.tile_size)

    def check_boss_defeat(self):
        """Verifica se o boss foi derrotado e cria o tesouro"""
        if not self.boss_group and not self.treasure_group:  # Se o boss morreu e não há tesouro
            treasure = Treasure(*self.treasure_pos)  # Cria o tesouro na posição guardada
    ...
    def check_collectibles(self):
        ...
        for coin in self.coins_group:
            if self.player.rect.colliderect(coin.rect):
                ...
                self.game.score_system.add_points("coin")
                coin.kill()
    ...
    def check_treasure_collision(self):
      """Verifica se o player colidiu com o baú e tem a chave"""
      for treasure in self.treasure_group:
          if self.player.rect.colliderect(treasure.rect):
              if treasure.try_open(self.keys > 0):  # Tenta abrir se tiver chave
                  self.keys -= 1  # Remove uma chave do inventário
                  self.collectibles_display.update_counts(coins=self.coins, keys=self.keys, potions=self.potions)
                  # Aqui você pode adicionar som de baú abrindo
                  # Toca o som do tesouro
                  self.treasure_sound.play()  
                  # Aqui você pode adicionar pontos por abrir o baú
                  self.score_system.add_points("treasure")

                  # Marca o nível como concluído
                  self.game.level_select.unlock_next_level(3) 
                   # Mostra tela de nível completo
                   # Mostra tela final
                  self.level_completed = True
                  self.screen_timer = pygame.time.get_ticks()  # Inicia o timer aqui
                  self.final_screen = FinalLevel(self.game, self.score_system.score)
                  #self.is_paused = True
    ...
    def run(self):
        ...
        # Inicia a música tema apenas se não estiver mudo
        if not self.audio_manager.is_muted:
            self.audio_manager.play_music("theme3")
        ...
        while running:
            ...
            # Atualiza e verifica o retorno do final_screen
            if self.level_completed and self.final_screen:
                action = self.final_screen.update()
                if action == "menu":
                    return "menu"        
            ...
            # Verifica game over
            if self.hearts <= 0 and not self.show_game_over:
                self.screen_timer = pygame.time.get_ticks()
                self.show_game_over = True
                self.game_over_screen = GameOver(self.game, self.score_system.score)
                #self.is_paused = True  # Pausa o jogo quando mostra game over
            current_time = pygame.time.get_ticks()
            # Para a tela de game over
            if self.show_game_over and not self.is_paused:
                if current_time - self.screen_timer >= self.screen_delay:
                    self.is_paused = True



            if self.level_completed and not self.show_final_screen:
                if current_time - self.screen_timer >= self.screen_delay:
            ...
            if not self.is_paused and not self.show_game_over:
                ...
                for boss in self.boss_group:
                    ...
                    for tile in self.tile_list:
                        if boss.rect.colliderect(tile[1]):
                            boss.reverse_direction()
                    
                  
                    
                    # Verifica colisão com lasers
                    for laser in boss.lasers:
            ...
            # Desenha o tesouro
            for treasure in self.treasure_group:
                screen_x = treasure.rect.x - self.camera_x
                screen_y = treasure.rect.y - self.camera_y
                if (-treasure.rect.width <= screen_x <= GameSettings["WINDOW_WIDTH"] and
                    -treasure.rect.height <= screen_y <= GameSettings["WINDOW_HEIGHT"]):
                    self.screen.blit(treasure.image, (screen_x, screen_y))
           ...
           #   # Se estiver em game over, desenha a tela e verifica ações
            ...
            if self.level_completed and self.show_final_screen and self.final_screen:
                self.final_screen.draw()
